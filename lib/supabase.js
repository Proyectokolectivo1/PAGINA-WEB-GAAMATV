...
export async function getNoticiaBySlug(slug) {
  try {
    ...
    // Try exact match first (published OR unpublished — auth/admin handles visibility)
    ...
    if (error || !noticia) {
      ...
      if (fallbackError || !fallbackNoticia) {
        ...
        if (normalizedError || !normalizedNoticia) {
          // Return null instead of throwing — callers handle not-found gracefully
          return null
        }
    ...
    // Fetch related data — each wrapped individually so a single failure
    // doesn't crash the entire page (secondary data is non-critical).
    if (noticia.categoria_id) {
      try {
        const { data: categoria } = await supabase
          ...
          .single()
        noticia.categoria = categoria || null
      } catch {
    ...
    // Catch-all: network errors, unexpected Supabase failures, etc.
    // Return null so callers show 404 instead of crashing with 500.
    console.error('[getNoticiaBySlug] Unexpected error for slug:', slug, err)
    return null
  }
}
...
export function extractYouTubeId(url) {
  ...
  // First, check if it's already a clean 11-char ID
  const cleanIdPattern = /^([a-zA-Z0-9_-]{11})$/
  const cleanMatch = cleanUrl.match(cleanIdPattern)
  if (cleanMatch) return cleanMatch[1]

  // Robust list of patterns covering:
  // - watch?v=ID, &v=ID
  // - youtu.be/ID
  // - embed/ID, shorts/ID, live/ID, v/ID
  // - youtube-nocookie.com variants
  const patterns = [
    /[?&]v=([^&\n?#\s]+)/,                              // watch?v=ID or &v=ID
    /youtu\.be\/([^&\n?#\s]+)/,                         // youtu.be/ID
    /youtube(?:-nocookie)?\.com\/embed\/([^&\n?#\s]+)/,   // embed/ID
    /youtube(?:-nocookie)?\.com\/shorts\/([^&\n?#\s]+)/,  // shorts/ID
    /youtube(?:-nocookie)?\.com\/live\/([^&\n?#\s]+)/,    // live/ID
  ...
  // Fallback: search for any "v=" or query/path containing 11 chars
...
/**
 * Resuelve la URL de imagen óptima para Open Graph (Facebook, Twitter, WhatsApp, etc.)
 *
 * Prioridad:
 *  1. imagen_principal de la noticia (convertida si es Google Drive)
 *  2. Thumbnail de YouTube si hay video_youtube_id
 *  3. null (el caller debe usar la imagen OG por defecto del sitio)
 *
 * Las URLs de Google Drive se enrutan a través del proxy /api/og-image
 * porque Facebook/WhatsApp no pueden acceder a drive.google.com directamente.
 * Las URLs de Cloudinary y YouTube son públicas y no necesitan proxy.
 *
 * @param {string|null} imagenPrincipal - URL de la imagen principal de la noticia
 * @param {string|null} videoYoutubeId - ID o URL del video de YouTube
 * @param {string} baseUrl - URL base del sitio (ej: https://gaamatv.com)
 * @returns {string|null} URL absoluta lista para og:image
 */
export function getOgImageUrl(imagenPrincipal, videoYoutubeId, baseUrl = '') {
  // 1. Intentar con la imagen principal
  if (imagenPrincipal) {
    const processed = processImageUrl(imagenPrincipal)
    if (processed) {
      // Si es Google Drive, pasar por el proxy para que los crawlers puedan accederla
      if (
        processed.includes('drive.google.com') ||
        processed.includes('googleusercontent.com')
      ) {
        return `${baseUrl}/api/og-image?url=${encodeURIComponent(processed)}`
      }
      // Cloudinary, Supabase storage, etc. — accesibles públicamente
      return processed
    }
  }

  // 2. Fallback: thumbnail de YouTube
  if (videoYoutubeId) {
    const ytId = extractYouTubeId(videoYoutubeId)
    if (ytId) {
      // maxresdefault puede no existir; hqdefault siempre está disponible
