...
export default function Footer() {
  ...
  return (
    <footer className="w-full border-t border-stone-200 bg-gradient-to-b from-stone-50 to-stone-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 lg:gap-12 mb-12">
          <div className="md:col-span-4">
            <Link href="/" className="inline-flex items-center gap-2 mb-6 group">
              <img 
                src="/logo-blanco.png" 
                alt="Gaama TV" 
                className="h-10 w-auto brightness-0 opacity-80 group-hover:opacity-100 transition-all"
              />
            </Link>
            <p className="font-body text-sm text-stone-500 leading-relaxed max-w-sm mb-6">
          ...
          <div className="md:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-6 md:gap-8">
            {ciudades.length > 0 && (
              <div>
                <h5 className="font-bold text-stone-900 mb-4 uppercase text-xs tracking-widest flex items-center gap-2">
                  <span className="w-2 h-2 bg-primary rounded-full"></span>
                  Ciudades
                </h5>
                <ul className="space-y-2.5 text-sm text-stone-500 font-label">
                  {ciudades.map(cat => (
                    <li key={cat.id}>
        ...
        <div className="pt-8 border-t border-stone-200 flex flex-col md:flex-row justify-between items-center gap-4 text-sm font-label text-stone-400">
